...
ReplaySource.prototype.end = function ReplaySource_end(t, x) {
  ...
  this._end(t, x, this)
  this.add(sink) // add an extra sink so the last values can go through
  setTimeout(this._end, 0) // dispose after values are propagated
};

ReplaySource.prototype._error = MulticastSource.prototype.end
...
function runProducer(t, buffer, sink) {
  const emit = item => {
    // console.log('emitting:', typeof item.x === 'number' ? item.x*10+5 : item.x)
